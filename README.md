...
The dataset is from processed CLM5-BGC simulations. The postprocessing involved resampling to 8-daily means for compatibility with many contemporary remote sensing products. The Plant Functional Type (PFT) level variables resolve the averaged grid cell time series for each present PFT. The relative coverage of the PFTs in each grid cell is indicated in the surface information variables (PCT_NAT_PFT), which must be scaled by the share of natural vegetation land unit in the grid cell (PCT_NAT_VEG). Similarly, the present crop types (PCT_CFT, unirrigated or irrigated) must be scaled with the present cropland land unit in the grid cell (PCT_CROP). Further information is provided by Poppe Terán et al. 2025, in the CLM5 description paper and the CLM5 technical note.
...
Poppe Terán, Christian; Belleflamme, Alexandre; Naz S., Bibi; Hendricks Franssen, Harrie-Jan; Vereecken, Harry, 2025, "European Water-Use Efficiency Dataset based on Community Land Model 5", https://doi.org/10.26165/JUELICH-DATA/UUCUVI, Jülich DATA, V1
